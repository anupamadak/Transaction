...
public class TransactionServiceImpl implements TransactionService {
	...
	/*
	 * @Autowired private ExpenseCategoryDao<ExpenseCategory> expenseCategoryDao;
	 */
	@Autowired
	...
	@Override
	public List<PdfDetails> readPdfFile(MultipartFile file) throws NCIException {
	...
	public void submitDetails(SubmitExpenseRequest request) {
		...
		for (Map.Entry<String, String> entry : details.entrySet()) {
			...
			if ("entertainment".equals(entry.getKey())) {
				expenseCategory.setEntertainment(Double.valueOf(entry.getValue()));
			}
			...
			if ("householdItems".equals(entry.getKey())) {
				expenseCategory.setHouseholdItems(Double.valueOf(entry.getValue()));
			}
