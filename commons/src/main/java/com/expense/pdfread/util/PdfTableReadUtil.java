...
import com.expense.exception.ErrorCode;
...
public final class PdfTableReadUtil {
	...
	public static List<PdfDetails> readPdfFile(MultipartFile file) {
		...
		try (PDDocument document = PDDocument.load(file.getBytes())) {
					...
					for (int r = 0; r < stripper.getRows(); ++r) {
						if (stripper.getText(r, c).toString().contains("Date")) {
							for (int i = 0; i < dateArray.length; i++) {
								String newDate = stripper.getText(r + i, c).toString().substring(0,
										stripper.getText(r + i, c).toString().length() - 3);
								if (!newDate.isEmpty()) {
						...
						if (stripper.getText(r, c).toString().contains("Description")) {

							for (int i = 0; i < descriptionArray.length; i++) {
								String newDate = stripper.getText(r + i, c).toString().substring(0,
										stripper.getText(r + i, c).toString().length() - 2);
								if (!newDate.isEmpty()) {
									descriptionArray[i] = stripper.getText(r + i, c);
								} else {
									break;
								}
							}
						...
						if (stripper.getText(r, c).toString().contains("Amount")) {
			...
			throw new NCIException("File is not uploaded", ioEx, ErrorCode.PDF_NOT_FOUND);
		}
